...
impl Chunk {
	...
	pub fn build_from_shapes(lod: u8, chunk_aabb: Aabb, shapes: &[&dyn Shape]) -> Self { todo!() }

	// Partially rebuild only the region covered by region_aabb, leaving the rest untouched.
	pub fn rebuild_region(&mut self, lod: u8, region_aabb: Aabb, shapes: &[&dyn Shape]) { todo!() }
}
