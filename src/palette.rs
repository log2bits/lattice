...
pub fn to_lms_cbrt(rgb: [u8; 3]) -> [f32; 3] {
	...
	[
		0.0514459929f32
			.mul_add(b, 0.4122214708f32.mul_add(r, 0.5363325363 * g))
			.cbrt(),
		0.1073969566f32
			.mul_add(b, 0.2119034982f32.mul_add(r, 0.6806995451 * g))
			.cbrt(),
		0.6299787005f32
			.mul_add(b, 0.0883024619f32.mul_add(r, 0.2817188376 * g))
			.cbrt(),
	]
}
...
pub fn lms_dist_sq(a: [f32; 3], b: [f32; 3]) -> f32 {
	...
	let dm = a[1] - b[1];
	let ds = a[2] - b[2];
	3.9574400593 * dl * dl + 7.1406209248 * dm * dm + 0.8570077675 * ds * ds
		- 9.2329090758 * dl * dm
		+ 1.7389374669 * dl * ds
		- 3.4610971558 * dm * ds
}
...
pub fn oklab_to_rgb(oklab: [f32; 3]) -> [u8; 3] {
	...
	[
		linear_to_srgb(
			(0.2309699292f32).mul_add(s, (4.0767416621f32).mul_add(l, -3.3077115913 * m)),
		),
		linear_to_srgb(
			(-0.3413193965f32).mul_add(s, (-1.2684380046f32).mul_add(l, 2.6097574011 * m)),
		),
		linear_to_srgb(
			(1.7076147010f32).mul_add(s, (-0.0041960863f32).mul_add(l, -0.7034186147 * m)),
		),
	]
}
